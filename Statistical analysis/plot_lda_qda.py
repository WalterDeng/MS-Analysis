...
def plot_data(lda, X, y, y_pred, target_names, title, is_plot_false=True):
    dimension = X.shape[1]
    if dimension == 1:
        plot_one_dimension_data(lda, X, y, y_pred, target_names, title, is_plot_false)
    elif dimension == 2:
        plot_two_dimension_data(lda, X, y, y_pred, target_names, title, is_plot_false)
    elif dimension >= 3:
        plot_three_dimension_data(lda, X, y, y_pred, target_names, title, is_plot_false)

def plot_two_dimension_data(lda, X, y, y_pred, target_names, title, is_plot_false=True):
    """
    ...
    # plt.scatter(X[-1, 0], X0_tp[-1, 1], marker="*", color="yellow", label="one-person-test")
...
def plot_three_dimension_data(lda, X, y, y_pred, target_names, title, is_plot_false=True):
    """
    传入训练好的数据、模型和预测值，画出三维分布图
    :param lda:
    :param X:
    :param y:
    :param y_pred:
    ...
    # class 1: dots
    ax1.scatter3D(X1_tp[:, 0], X1_tp[:, 1], X1_tp[:, 2], marker="^",s=30, edgecolors="red", c="", label=target_names[1])
    if is_plot_false and X1_fp.shape[0]>0:
        ax1.scatter3D(X1_fp[:, 0], X1_fp[:, 1], X1_fp[:, 2], marker="x", s=20, color="#990000", label=false_names[1])  # dark blue
    # ax1.plot3D(x, y, z, 'gray')  # 绘制空间曲线

    # class 0: dots
    ax1.scatter(X0_tp[:, 0], X0_tp[:, 1], X0_tp[:, 2],marker="o", edgecolors='blue', color="", label=target_names[0])
    if is_plot_false and X0_fp.shape[0]>0:
        ax1.scatter(X0_fp[:, 0], X0_fp[:, 1], X0_fp[:, 2],marker="x", s=20, color="#000099", label=false_names[0])  # dark red
...
def plot_one_dimension_data(lda, X, y, y_pred, target_names, title, is_plot_false=True):
    """
    给1维的X在纵轴上生成1-5的随机数，从而可视化1维分类结果
    :param lda:
    :param X:
    :param y:
    :param y_pred:
    ...
    :return:
    """
    zz = np.random.rand(X.shape[0], 1) * 2
    X = np.concatenate((X, zz), axis=1)
