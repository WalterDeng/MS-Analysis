...
import matplotlib.pyplot as plt
def LassoLarsCV_plot(X, y, ymin=0, ymax=1):
    # start_time = time.time()
    # model = make_pipeline(StandardScaler(), LassoLarsCV(cv=20, normalize=False)).fit(X, y)
    # fit_time = time.time() - start_time

    # %%
    lasso = LassoLarsCV(cv=20, normalize=False)
    lasso.fit(X, y)
    ...
    print("alpha值为：", lasso.alpha_)
    plt.ylim(ymin, ymax)
    plt.xlim(10e-8, 10e-5)
    plt.xlabel(r"$\alpha$")
    plt.ylabel("Mean square error")
    plt.legend()
    _ = plt.title("Mean square error on each fold: Lars (total: 20 fold)")
    plt.show()
    coef_ = lasso.coef_
    markers_index = coef_ > 0
    return markers_index

# LassoLarsCV_plot(X, y)
