...
void data_preprocessing_api(const mxArray * const prhs[1], int32_T nlhs, const
  mxArray *plhs[3])
{
  ...
  emxInit_real_T(&st, &atlas_data, 2, true);
  emxInit_real_T(&st, &data_baselined, 2, true);
  emxInit_real_T(&st, &base, 2, true);
  emxInit_real_T(&st, &peaks_info, 2, true);

  /* Marshall function inputs */
  atlas_data->canFreeData = false;
  emlrt_marshallIn(&st, emlrtAlias(prhs[0]), "atlas_data", atlas_data);
  ...
  emxFree_real_T(&data_baselined);
  emxFree_real_T(&atlas_data);
  if (nlhs > 1) {
