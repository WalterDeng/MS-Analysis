...
% 输出 base : (feature_num)
    ...
    peak_smooth_window = ceil(frequency)*1.5;
...
%                     y2 = (data(ii,x2) + data(ii,x2+1)) /2;  % 前面默认取最后一个点，此处取�?个均�?
            ...
            [~,I1] = min(abs(sx(ii,x1:x2)-(y2*0.5)));%The closest point left of apex.
            [~,I2] = min(abs(sx(ii,x2:x3)-(y2*0.5)));%The closest point right of apex.
            
            w = time_series(x2+I2) - time_series(x1+I1);
            % 判断峰宽、峰高是否符合要�?
            if w>min_half_width && area>min_area && y2>min_peak_height
...
%                     P = [x1, time_series(x2), x3, y2, w, area]; % delete
                    peaks_i = peaks_i + 1;
